...
# Initialize client. Can be done globally or per request.
# For serverless functions or apps that might scale, initializing per request or with context management is safer.
# However, for simplicity in this Streamlit app, a global client can work if HF_TOKEN is present.
# If HF_TOKEN is not set, client initialization here would fail if it tries to validate token immediately.
# The InferenceClient can be initialized without a token, but calls will fail.
# Let's initialize it within the function call to ensure HF_TOKEN is checked closer to the actual call.
...
def get_creative_idea(skill1: str, skill2: str, skill3: str = None) -> dict:
    ...
    try:
        ...
        # Assuming the response object structure is OpenAI-compatible as per HF docs
        ...
        # Check for common issues like model loading/availability if possible from error
    ...
    # Basic parsing - this will likely need refinement based on actual model output
    parsed_output = {}
    key_mapping = {
    ...
    lines = content.split('\n')
    # If content is short or no newlines, it might be a direct answer or an error message from the model itself
    if not lines or (len(lines) == 1 and not any(key in lines[0] for key in key_mapping)):
        # If it seems like a plain sentence rather than our structured output
    ...
    for line in lines:
        ...
        for key_prompt, key_dict in key_mapping.items():
            # Check for "Key: Value" or "1. Key: Value"
            if line_stripped.lower().startswith(key_prompt.lower() + ":"):
                ...
                found_key = True
                break
        # Simple continuation logic (if a line doesn't start with a known key, append to the last found key)
        # This is very basic and might need improvement.
        # if not found_key and parsed_output and last_found_key_dict:
        #    parsed_output[last_found_key_dict] += " " + line_stripped
    ...
    if parsed_output.get("project_title", "Not specified by AI.") == "Not specified by AI.":
        # If title is missing, parsing likely failed or model response was poor.
        # Return raw content in description for debugging.
        parsed_output["project_description"] = f"AI response (parsing failed or incomplete): {content}"
        # Consider returning an error if title is absolutely essential and not found
        # return {"error": "Failed to parse project title from model response.", "raw_content": content}
...
if __name__ == '__main__':
    ...
    if not HF_TOKEN:
        ...
        if "error" in idea1:
            ...
            if "raw_content" in idea1: # If we add raw_content to error dict
                 print(f"Raw Content:\n{idea1['raw_content']}")
        else:
